...
export class CvsService {
  ...
  /**
   * Resolve the original uploaded file for a CV so the controller can
   * stream it back to the candidate's, recruiter's, or admin's browser.
   *
  ...
  async getFileInfo(
    ...
    if (!cv.filePath || cv.source === 'builder') {
      throw new NotFoundException(
